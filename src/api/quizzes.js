...
export const getAllQuizNames = async () => {
  const data = await axios.get(`quizzes/names`).then((res) => res.data);
  return data;
};

export const createQuiz = async ({ title, description, img, category }) => {
  axios.post(`quizzes`, {
    title,
    description,
    img,
